...
2. **Dynamic content route + About activation** (C2): `src/pages/[page].astro` with `getStaticPaths()` constrained through `PAGES` + `resolveRoutes` (not all routable collection entries — unknown public records must NOT generate routes). `getCollection("pages")` → `assertMarkdownRendered(entry)` on every entry → `resolveRoutes` → join through `PAGES` → `render(entry)` → `<Content />` inside BaseLayout. About ships via two microcycles: (1) `noindex` to prove the dynamic route derives `noindex` from visibility, then (2) promoted to `public`. Verifier CLI manifest + inherited B1/B2/B3 shell tests updated atomically (B2's "no about links" assertion becomes "about link present, services/contact absent"; manifests add `/about/`). A permanent copied-production noindex variant is retained in the final suite.
