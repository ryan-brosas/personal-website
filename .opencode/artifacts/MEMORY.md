...
- A Conventional-Commits hook pattern-matches the literal substrings `git commit` and `git push` anywhere in a shell command, not just at the start. A `grep`/`sed`/`echo`/`python -c` containing those literals is blocked with `Blocked: git commit missing -m message`. Verify with `read`, `rg` patterns that spell around them, or `git diff --check`. Discovered 2026-07-22.
