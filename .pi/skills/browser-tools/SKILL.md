...
Run helpers through Pi shell execution with explicit project-local paths:

```bash
node .pi/skills/browser-tools/browser-start.js
node .pi/skills/browser-tools/browser-nav.js https://example.com
node .pi/skills/browser-tools/browser-screenshot.js
node .pi/skills/browser-tools/browser-content.js https://example.com
```

For skill-scoped MCP automation instead, load `/skill:playwright` or `/skill:chrome-devtools` and call the registered `skill_mcp` tool.
