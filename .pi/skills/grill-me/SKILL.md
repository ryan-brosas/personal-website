...
This skill is an iterative, turn-by-turn interview — one question at a time, waiting for the operator's answer before the next. That loop is **primary-session-only**: only the primary session (the operator-facing conversation) may run it, and it must **not** be delegated to a Fabric subagent / leaf worker, which breaks the back-and-forth. A subagent / leaf that discovers grilling is required must **not** run the interview itself — it reports a blocker in its result envelope stating that grilling is required and escalates to the primary so the primary can drive the interview. Leaves may still perform read-only context exploration (files, docs, recent commits) to scope the request before reporting.
