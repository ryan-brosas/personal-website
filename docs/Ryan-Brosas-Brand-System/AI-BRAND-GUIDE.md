...
- Evidence confidence: **Verified**, **Proposed**, or **Open**.
- Operational lifecycle: default, loading or validating, content or empty, error, and recovery.
- Interaction state: hover, pressed, selected, focus-visible, and disabled.

Verified means an inspected source supports the exact sentence, not merely that a file exists. Name the source and limit the claim to what it shows. Proposed language names the recommendation and its tradeoff. Open language starts with **Still needed:** and names the owned proof plus the decision it blocks. Do not use Open styling for an operational error or Verified styling as a generic success state.

### AI-authored copy contract

- Name the smallest reliable intervention as an **agent**, **script**, or **process change**.
- Sequence generated copy as operating consequence → system choice → tradeoff → evidence boundary → next action.
- Reject a Verified sentence without a named inspected source.
- Reject a Proposed recommendation without its tradeoff.
- Reject an Open statement that does not name the missing owned proof and the decision it blocks.
- Reject error copy that omits what remains safe or the next available action.

Use this reusable sentence grammar when a draft becomes vague:
...
Replace every bracketed field with inspected evidence or keep the statement explicitly Open. These patterns are operating grammar, not reusable claims.
...
5. Apply one theme attribute to the owned root; do not hard-code a second dark palette inside each component. When a dark theme is nested inside a light document, rebind every mode-dependent semantic, compatibility, rule, shadow, and elevation alias in the final dark token scope.
...
14. Index every canonical applied artifact intended for Design System Assets. Keep the landing page as the primary preview, register the client pitch, campaign poster, service email, build-breakdown newsletter, and workflow service inquiry as separate preview pages with sidecars, and generate `system/artifacts/deck.html`, `system/artifacts/poster.html`, `system/artifacts/email.html`, `system/artifacts/newsletter.html`, and `system/artifacts/form.html` with package resources rebased two levels for their fixed renderer slots.
15. For evidence lifecycle components, consume the matching background, foreground, border, and muted-caption roles from the same `--evidence-[state]-*` family. Persistent selections use `--control-selected-*`; they never borrow Open styling.
16. For fixed 4:5 social work, consume the `--campaign-*` canvas, accent field, rule, metadata, safe-area, title, art-scale, and footer roles before adding local composition values. Keep narrative art fully above durable footer language and remove export-only labels from the published composition.
17. Treat each component family as a complete contract. Consume `--feature-border`, `--feature-surface`, `--feature-meta`, `--nav-overlay-bg`, `--action-disabled-opacity`, and the matching `--feedback-*-muted` role directly; do not reconstruct registered behavior with local `color-mix()` or compound semantic text with blanket opacity.
18. Treat the System Map as its own complete component family. Consume `--system-map-*` for its surface, text, steps, index, connector, and recovery loop; preserve the same five questions and recovery-to-context note in both modes.
