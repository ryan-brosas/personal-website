...
> Package version: `1.10.3` — dedicated System Map roles, reusable copy grammar, and exact runtime inventory now pass the readiness contract  
...
Component families are complete contracts. A feature plate consumes its background, foreground, muted copy, border, nested surface, and metadata roles; a sticky navigation surface consumes `--nav-overlay-bg`; a disabled task button consumes its semantic colors plus `--action-disabled-opacity`; feedback states consume their matching background, foreground, border, and muted-detail roles. Do not rebuild these values with local `color-mix()` or compound semantic text colors with blanket opacity.
...
When any semantic, compatibility, rule, shadow, or elevation alias depends on a mode role, declare it again in the final dark block after the unscoped foundation tokens. This includes component backgrounds, foregrounds, strokes, selected states, disabled states, evidence lifecycle roles, `--surface-interactive-*`, legacy reading aliases, `--rule*`, and `--elev-*`. The final rebinding keeps a nested `[data-theme="dark"]` kit correct even when its parent document remains light; inherited light-resolved aliases must never leak into the contained dark surface.
...
- Disabled: use `--action-disabled-*` plus `--action-disabled-opacity` and `not-allowed`; do not compound already-muted text with a second blanket opacity.
...
Feedback has three semantic treatments without adding decorative hues: neutral uses `--feedback-neutral-*`, error uses `--feedback-error-*`, and notice uses `--feedback-notice-*`. Each family owns background, foreground, border, and muted detail. Bind message-body copy to `--feedback-neutral-muted`, `--feedback-error-muted`, or `--feedback-notice-muted`; never lower the entire text layer with local opacity. Every message writes the state in words. Error and recovery copy follows **what happened → what remains safe → next available action**. Use `role="status"` for non-urgent updates and `role="alert"` only when immediate attention is required.
...
Consume the complete `--system-map-*` family for the map surface, foreground, muted question copy, border, step surface, index, connector, recovery-loop surface, loop text, and loop border. Do not reconstruct this registered component from `--panel-*`, `--surface-*`, `--text-*`, or local `color-mix()` values. The same anatomy and reading order must appear in light and dark mode.
...
- `system/kit.dark.html` uses a relative `./kit.html?theme=dark` iframe entry. Never point it back to the design-system Showcase API route, which can recurse through the dark renderer.
...
Do not reconstruct a registered component family from lower-level surfaces or local `color-mix()`. If a reusable anatomy needs a distinct border, nested surface, overlay, metadata tone, or disabled-opacity behavior, add that role to `colors_and_type.css`, rebind it in dark mode when necessary, list it in `system/theme.json` and `component-manifest.json`, then consume it directly.
...
- **Verified:** name the inspected file or owned capture, then state only the exact evidence it supports.
...
Use these sentence shapes when a draft becomes abstract:

- Consequence: `[Recurring work] keeps returning because [context, check, or handoff is missing].`
- Choice: `Use a [script, process change, or agent] when [the condition that makes it reliable].`
- Tradeoff: `This removes [recurrence], but adds [maintenance, review, or ownership cost].`
- Verified: `[Inspected source] shows [bounded fact], so [safe use].`
- Open: `Still needed: [owned proof] before [decision or publication].`
- Action: `[Concrete verb] [specific object].`

These are operating grammars, not slogans. Replace every bracketed field with a real fact or keep the sentence Open.
