(function () {
  ...
  function FormField(options) {
    ...
    var attrs = [
      ...
      'value="' + safe(options.value == null ? "" : options.value) + '"',
    ...
    if (options.message) attrs.push('aria-describedby="' + safe(id) + '-message"');
    if (state === "invalid") {
      attrs.push('aria-invalid="true"');
    }
    if (state === "validating") attrs.push('aria-busy="true"');
    return [
  ...
  function EvidenceMedia(options) {
    ...
    var icon = state === "error" ? "alert" : state === "loading" || state === "recovery" ? "refresh" : state === "verified" ? "check" : "file-plus";
    var busy = state === "loading" ? ' aria-busy="true"' : "";
    return [
      '<figure class="evidence-media evidence-media--' + safe(state) + '" data-evidence-state="' + safe(state) + '" data-od-id="kit-evidence-' + safe(options.id || state) + '"' + busy + '>',
      ...
      options.actionLabel ? window.Button({ kind: "secondary", label: options.actionLabel, action: options.action || "recover-evidence", odId: "kit-evidence-action-" + (options.id || state), icon: options.actionIcon || "refresh" }) : "",
      '</figure>'
    ].join("");
  }
