...
For product and workbench interfaces, load `colors_and_type.css` and consume its semantic roles instead of recreating the palette. Set `data-theme="light"` or `data-theme="dark"` once on the owned root. Components use `--surface-*`, `--text-*`, `--stroke-*`, `--action-*`, `--panel-*`, `--field-*`, `--status-*`, and `--nav-*`; the four source pigments remain fixed in both modes.
...
2. Load root `tokens.css` before component CSS. It is generated from and byte-identical to `colors_and_type.css`.
3. Read `component-manifest.json` for anatomy, state, accessibility, responsive, and content-integrity requirements.
4. Apply one theme attribute to the owned root; do not hard-code a second dark palette inside each component.
5. Use the preserved SVG that matches the background.
6. Check the applied reference under `ui_kits/app/` before inventing a new component shape.
7. For Open Design delivery, keep `manifest.json`, `tokens.css`, `system/kit.html`, `system/kit.dark.html`, `system/variables.css`, `system/theme.json`, and `logos/` synchronized. `tokens.css` activates the runtime token channel; both token mirrors must declare values directly and match the canonical authoring source.
8. Compare the light and dark role proofs side by side. Do not fix one mode with component-specific hard-coded colors.

The component registry covers navigation, buttons, status panels, form fields, the five-part system map, project-evidence requests, logo tiles, theme controls, recovery feedback, and evidence media. Extend it only when source or product needs provide a real new pattern, and do not distribute unreferenced prototype components under the registered system id.
