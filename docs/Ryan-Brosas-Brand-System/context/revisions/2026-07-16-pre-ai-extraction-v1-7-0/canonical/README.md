...
3. Load `tokens.css` (or the identical authoring source `colors_and_type.css`), then apply `data-theme="light"` or `data-theme="dark"` to the owned root.
...
13. Confirm the registered `ui_kits/app/components/` directory contains only the five files referenced by the applied kit and component manifest.
...
`colors_and_type.css` is the authoring source. Root `tokens.css` and `system/variables.css` are full generated mirrors: the root file activates Open Design's normalized token channel, while the system file preserves the legacy package surface. Never edit the three independently; regenerate both mirrors and verify byte parity before publishing.
