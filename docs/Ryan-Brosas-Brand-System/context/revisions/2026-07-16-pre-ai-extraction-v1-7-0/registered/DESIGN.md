...
> Package version: `1.6.2` — normalized runtime manifest, live token channel, and repaired Showcase asset delivery  
...
3. **Component roles** — `--action-*`, `--panel-*`, `--field-*`, `--status-*`, `--feedback-*`, `--link-*`, `--evidence-*`, `--state-selected-*`, `--feature-*`, and `--focus-*`. Components consume these names rather than reconstructing colors.
...
`colors_and_type.css` is the canonical authoring source. Root `tokens.css` and `system/variables.css` are generated, self-contained mirrors. The root mirror activates Open Design's token channel; the system mirror preserves the legacy package surface. Regenerate both from the canonical file and verify declaration parity—never hand-maintain divergent token sheets. `system/theme.json` provides explicit token inventories and must not use wildcard names that require consumer-specific expansion.
