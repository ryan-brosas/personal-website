...
5. Apply `data-theme="light"` or `data-theme="dark"` to the owned root and consume semantic and component roles. Use `--action-*` for task buttons and `--control-*` for segmented preferences, filters, pressed states, and persistent selections.
...
20. For fixed 4:5 campaign work, consume the `--campaign-*` canvas, accent field, rule, metadata, safe-area, and title roles. Keep the landing page primary when registering the campaign asset.
...
- **Campaign:** fixed 1080 × 1350 surface, safe-area, coral-field, metadata, and title roles that remain stable across surrounding document themes.
...
- **Voice:** casual, nerdy, specific, honest, and failure-aware without hype; landing pages move from operating consequence to workflow tension, mechanism, evidence boundary, and a specific action.
