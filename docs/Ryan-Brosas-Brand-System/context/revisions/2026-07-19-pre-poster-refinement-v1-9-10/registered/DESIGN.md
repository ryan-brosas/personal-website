...
> Package version: `1.9.9` — state ownership now reaches lifecycle captions and readiness controls, while fixed 4:5 campaign roles replace poster-local values  
...
All twelve approved clean-print files were re-checked at `1254 × 1254` with ARGB transparency. The client pitch deck remains `54,138` bytes with 18 labelled slides, 95 inspectable `data-od-id` hooks, seven distinct approved illustration references, one active slide, and six written Open labels. The refined social poster is `7,445` bytes on a fixed `1080 × 1350` canvas. The linked Figma node remains structural provenance: direct page retrieval and indexed search were blocked in this pass, so no external value was guessed or promoted. The re-measured local HTML, CSS, SVGs, palette asset, component previews, owned renders, deck, and poster remain authoritative.
...
The canonical social campaign proof is `building-systems-social-poster.html`, a fixed 1080 × 1350 composition for “Building systems so everything doesn’t need you.” It uses the System Conductor as narrative art, not operating evidence; keeps the full illustration inside the canvas; uses the charcoal mark on its protected signal-yellow tile; loads root `tokens.css` once; and consumes the fixed `--campaign-*` surface, safe-area, accent-field, and title roles. Keep this poster indexed after the primary landing page and the client-pitch asset.
...
22. Confirm Design System Assets exposes `showcase-landing-page.html`, `founder-dependence-client-pitch-deck.html`, and `building-systems-social-poster.html`; the landing page remains primary, every artifact has its sidecar, `system/artifacts/landing.html` and `system/artifacts/deck.html` exist in canonical and registered packages, and every nested renderer resource resolves after its two-level path rebase.
