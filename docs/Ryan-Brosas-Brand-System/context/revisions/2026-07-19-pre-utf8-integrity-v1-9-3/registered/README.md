...
Version 1.9.2 turns the measured founder-dependence client deck into a reusable presentation contract without creating a second design system. Fixed dark, light, and Open presentation roles now sit beside the workbench mode roles; deck body, metadata, canvas, and safe-area values are explicit; and the typography, light/dark, voice, and applied-surface proofs document the boundary between narrative slides and runtime components. The verified palette and existing named heading scale remain unchanged. The package audit again preserves and removes six reintroduced JSX prototypes so only the seven registered runtime files remain active.
