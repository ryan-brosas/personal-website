...
> Package version: `1.9.0` — the source evidence is re-measured, maintainer and consumer token entries are explicit, segmented-control roles are dedicated, and the applied kit visibly exercises the full lifecycle contract  
...
The approved clean-print hero and sampled operational illustrations were also re-checked at `1254 × 1254` with ARGB transparency. The linked Figma node remains a structural reference, but direct retrieval and indexed search were blocked by Figma access controls and robots policy in this run, so no value was inferred from it. No linked product website, repository, or external source folder is declared by the source context. The re-measured local HTML, CSS, SVG, palette asset, component previews, and rendered evidence therefore remain authoritative.
