...
- `component-manifest.json` — machine-readable anatomy, token, state, accessibility, responsive, recovery, and content-integrity contracts for reusable components.
...
2. Read `component-manifest.json` before adapting navigation, controls, forms, status panels, system maps, evidence cards, or logo tiles.
3. Load `colors_and_type.css`, then apply `data-theme="light"` or `data-theme="dark"` to the owned root.
4. Inspect the focused `preview/` cards, especially light/dark roles, real logos, and component states.
5. Open `ui_kits/app/index.html` and exercise theme and track switching, mobile navigation disclosure, section filters, copy feedback, and checklist states.
...
6. Compare the reusable kit against `source_examples/brand-workbook/` and the preserved screenshots.
7. Copy only the components and assets needed for a new project; preserve exact logo geometry and asset filenames.
8. Keep missing project media labelled Open until owned evidence is supplied.
9. Before claiming an in-place refinement is live, confirm the registered package contains `system/kit.html`, all four files under `logos/`, the current `DESIGN.md`, and published metadata for the unchanged id.
10. Confirm `system/variables.css` contains declarations directly, its token names match `colors_and_type.css`, and `system/theme.json` contains no wildcard role names.
11. Confirm the registered `ui_kits/app/components/` directory contains only the five files referenced by the applied kit and component manifest.
...
`colors_and_type.css` is the authoring source. `system/variables.css` is intentionally a full generated mirror rather than an import-only wrapper because registered token extractors may not follow nested imports. Never edit the two independently; regenerate the packaged file and verify token-name parity before publishing.
