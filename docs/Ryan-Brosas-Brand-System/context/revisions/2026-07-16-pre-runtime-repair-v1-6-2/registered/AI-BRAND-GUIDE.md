...
7. For Open Design Showcase delivery, keep `system/kit.html`, `system/kit.dark.html`, `system/variables.css`, `system/theme.json`, and `logos/` synchronized with the root package. The packaged variables file must declare tokens directly and match the canonical token names; an import-only wrapper is not sufficient.
