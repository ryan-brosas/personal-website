...
> Package version: `1.6.1` — self-contained packaged tokens, explicit machine inventories, and audited distribution readiness  
...
The current owned workbook CSS measures a `1480px` maximum workbench width, an 8px baseline with 4px optical adjustments, `0px`/`4px`/`8px` radii plus status-only pills, and responsive changes at `1024px`, `820px`, and `600px`. The three source page renders are `1800px` wide. Every adopted SVG preserves a `0 0 255 211` viewBox and is byte-identical to its copy under `assets/`.

The linked Figma node remains a structural reference. During the 1.6 refinement, live Figma extraction was unavailable because the connected Starter plan had reached its MCP call limit, so no unverified Figma measurement was promoted over the owned HTML, CSS, SVG, and rendered evidence.
...
2. **Mode roles** — `--canvas`, `--surface-1`, `--surface-2`, `--surface-3`, `--surface-inverse`, `--workbench-canvas`, `--presentation-canvas`, `--surface-interactive-*`, `--text-1`, `--text-2`, `--text-3`, and `--stroke-*`. These change under `[data-theme="dark"]`.
3. **Component roles** — `--action-*`, `--panel-*`, `--field-*`, `--status-*`, `--feedback-*`, `--link-*`, `--evidence-*`, `--state-selected-*`, `--feature-*`, and `--focus-ring`. Components consume these names rather than reconstructing colors.
...
Owned captures use `--evidence-media-*` for the plate and `--evidence-caption-*` for the source label and explanation. Preserve the source aspect ratio until a crop is documented. Keep captions outside clipping media regions. An Open media request stays visibly Open until owned evidence replaces it; a polished placeholder is not proof.
...
4. Build components from semantic and component roles rather than source pigments.
5. Load the correct production SVG from `logos/`.
6. Populate content using the Verified/Proposed/Open taxonomy.
7. Add real states, recovery copy, and responsive behavior before visual polish.
8. Check the light and dark contracts side by side before treating either mode as complete.

### Showcase and registered-package contract

The Open Design Showcase reads `system/kit.html` from the registered design-system directory. It does not infer the Showcase from `preview/` files. Keep these surfaces synchronized:
...
- `logos/` — the exact four adopted SVGs referenced by the Showcase.
...
`colors_and_type.css` is the canonical authoring source. `system/variables.css` is a generated, self-contained mirror because registered consumers may not resolve nested `@import` statements. Regenerate it from the canonical file and verify declaration parity; never hand-maintain a divergent second token sheet. `system/theme.json` provides explicit token inventories and must not use wildcard names that require consumer-specific expansion.
...
13. Confirm `system/variables.css` declares the same token names as `colors_and_type.css` without relying on `@import`.
14. Confirm `system/theme.json` lists concrete token names and the registered distribution contains no unreferenced off-brand prototypes.
