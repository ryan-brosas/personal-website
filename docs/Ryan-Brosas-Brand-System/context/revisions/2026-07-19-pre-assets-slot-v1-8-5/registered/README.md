...
Version 1.8.4 fixes Design System Assets discoverability. `showcase-landing-page.html` is a generated mirror of the canonical `ryan-brosas-landing-page.html` and is declared first as the package’s indexed Showcase preview, so the Assets browser opens the applied landing page instead of `preview/index.html`. The canonical page remains the only independently edited source.
