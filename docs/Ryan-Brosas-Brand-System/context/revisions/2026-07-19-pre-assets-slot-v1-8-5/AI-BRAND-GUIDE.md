...
Use `ryan-brosas-landing-page.html` as the living applied proof for the asset system. It should continuously demonstrate the identity, hero, editorial, and interface layers together. Update the same page, regenerate `showcase-landing-page.html` as the indexed Design System Assets mirror, and refresh its registered capture after an accepted revision; do not multiply one-off landing variants unless a separate direction is explicitly requested.
