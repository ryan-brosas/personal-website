...
> Package version: `1.9.1` — calm named typography roles replace oversized local clamps, source evidence remains measured, and the applied and registered kits share one exact runtime contract  
...
The local source set was re-measured on 2026-07-19: the two workbook stylesheets contain `25,232` UTF-8 bytes, the three canonical HTML pages contain `51,590` UTF-8 bytes, and the responsive sequence remains `1024px`, `820px`, and `600px`. The current pages contain 38 panel instances, 35 written status instances, 38 compact labels, 11 copy controls, nine token rows, six project cards, five system steps, and four logo plates. The system still uses a `1480px` maximum workbench width, an 8px baseline with 4px optical adjustments, `0px`/`4px`/`8px` radii plus status-only pills, and `1800px` review renders. Every adopted SVG preserves a `0 0 255 211` viewBox and is byte-identical to its copy under `assets/`.

The approved clean-print hero and sampled operational illustrations were also re-checked at `1254 × 1254` with ARGB transparency. The linked Figma node remains a structural reference, but direct retrieval and indexed search were blocked by Figma access controls and robots policy in this run, so no value was inferred from it. The user-linked `kopywriting.com` page was re-read as an inspiration reference: its accessible structure supports compact sequential headings, direct explanatory paragraphs, and explicit next actions. Cloudflare blocked direct stylesheet retrieval, so no external font, color, spacing, or metric was copied. The re-measured local HTML, CSS, SVG, palette asset, component previews, and rendered evidence remain authoritative.
