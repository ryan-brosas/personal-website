...
- `colors_and_type.css` exposes implementation-ready tokens.
- `assets/` contains four production SVG marks, compatibility rasters, palette evidence, the contained reference board, and source review images.
...
- `ui_kits/app/` contains a reusable applied workbench and modular browser-ready components.
...
3. Load or copy `colors_and_type.css`; do not recreate colors by eye.
4. Apply `data-theme="light"` or `data-theme="dark"` to the owned root and consume semantic roles.
5. Choose the correct SVG from `assets/` based on background contrast.
6. Inspect the relevant focused card in `preview/` for the part being built.
7. Reuse or adapt components from `ui_kits/app/components/` and compare them with `source_examples/brand-workbook/`.
8. Keep Brand Strategy, Brand Presentation, and Brand Guidelines as the primary pages.
9. Label facts as Verified, recommendations as Proposed, and missing inputs as Open.
10. Validate both themes, responsive layout, 44px targets, focus-visible behavior, reduced motion, and optical logo centering before delivery.
...
- **Interaction:** real hover, focus-visible, selected, disabled, loading, invalid, theme preference, copy-feedback, and reduced-motion states.
