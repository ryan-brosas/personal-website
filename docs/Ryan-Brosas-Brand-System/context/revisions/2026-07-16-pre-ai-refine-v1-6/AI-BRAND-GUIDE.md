...
7. For Open Design Showcase delivery, keep `system/kit.html`, `system/kit.dark.html`, `system/variables.css`, `system/theme.json`, and `logos/` synchronized with the root package.

The component registry covers navigation, buttons, status panels, form fields, the five-part system map, project-evidence requests, and logo tiles. Extend it only when source or product needs provide a real new pattern.

The Showcase is a product-facing proof surface, not a generic demo page. It must show the adopted logo, real brand copy, the Verified/Proposed/Open taxonomy, both semantic modes, and component states without invented clients, metrics, testimonials, or outcomes.
