...
Version 1.8.0 closes the preview-versus-implementation gap. `Primitives.js` now owns buttons, utility icons, logo tiles, status labels, and theme controls; `LifecycleComponents.js` owns form fields, feedback, and evidence media. Editable sources live in `ui_kits/app/components/`; byte-identical consumable copies live under the registered `assets/components/` boundary. The applied kit loads those registered copies before composed components and visibly renders them. Six off-brand JSX prototypes were removed from the active distribution and preserved in `context/revisions/2026-07-19-pre-ai-refine-v1-8-0/`.
...
Version 1.8.0 preserves the v1.7.2 dark-mode correction and adds named disabled borders, status-muted roles, field hints, and complete evidence empty/error/recovery roles. The nine downstream previews and the applied kit load `tokens.css` as the single consumer entry.
