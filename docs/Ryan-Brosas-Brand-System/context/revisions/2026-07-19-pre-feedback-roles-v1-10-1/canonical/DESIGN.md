...
> Package version: `1.10.0` — component roles now own feature, overlay, and disabled-action behavior end to end  
...
Component families are complete contracts. A feature plate consumes its background, foreground, muted copy, border, nested surface, and metadata roles; a sticky navigation surface consumes `--nav-overlay-bg`; a disabled task button consumes its semantic colors plus `--action-disabled-opacity`. Do not rebuild these values with local `color-mix()` or compound semantic text colors with blanket opacity.
