...
17. Treat each component family as a complete contract. Consume `--feature-border`, `--feature-surface`, `--feature-meta`, `--nav-overlay-bg`, and `--action-disabled-opacity` directly; do not reconstruct registered behavior with local `color-mix()` or compound semantic disabled text with blanket opacity.
