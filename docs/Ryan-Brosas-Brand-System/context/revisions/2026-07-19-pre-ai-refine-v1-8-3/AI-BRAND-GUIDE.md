...
5. Apply one theme attribute to the owned root; do not hard-code a second dark palette inside each component.
...
8. For Open Design delivery, keep `manifest.json`, `tokens.css`, `system/kit.html`, `system/kit.dark.html`, `system/variables.css`, `system/theme.json`, and `logos/` synchronized. `tokens.css` activates the runtime token channel and is the only token sheet downstream artifacts load; maintainers edit `colors_and_type.css`, then regenerate both mirrors with direct declarations and byte parity.
9. Compare the light and dark role proofs side by side. Do not fix one mode with component-specific hard-coded colors.
...
Use `ryan-brosas-landing-page.html` as the living applied proof for the asset system. It should continuously demonstrate the identity, hero, editorial, and interface layers together. Update the same page and refresh its registered capture after an accepted revision; do not multiply one-off landing variants unless a separate direction is explicitly requested.
