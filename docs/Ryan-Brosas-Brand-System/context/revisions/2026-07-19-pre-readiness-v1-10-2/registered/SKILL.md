...
5. Apply `data-theme="light"` or `data-theme="dark"` to the owned root and consume complete semantic component families. Use `--action-*` for task buttons, `--control-*` for persistent selections, `--feature-*` for feature anatomy, and `--nav-overlay-bg` for translucent navigation; never rebuild registered behavior with local `color-mix()`.
