...
> Package version: `1.7.2` — dark-mode recovery, field, panel, navigation, and evidence foreground roles are explicitly resolved  
...
The linked Figma node remains a structural reference. It was retried through the authenticated connector on 2026-07-16, but the connected Starter plan still returned its MCP tool-call limit; public fetch was also blocked. No Figma value was guessed or promoted over the owned HTML, CSS, SVG, palette asset, component previews, and rendered evidence.
...
Use `assets/icons.svg` before adding a new icon dependency. The collection is a 24px `currentColor` outline system with a 1.75px stroke and round caps/joins: menu, close, arrow-right, chevron-down, copy, check, refresh, alert, info, and file-plus. Use icons only for navigation, disclosure, direction, copy, recovery, evidence request, or reinforcement of a written state.
