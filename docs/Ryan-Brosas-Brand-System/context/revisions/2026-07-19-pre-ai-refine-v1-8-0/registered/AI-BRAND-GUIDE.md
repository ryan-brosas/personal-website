...
7. For Open Design delivery, keep `manifest.json`, `tokens.css`, `system/kit.html`, `system/kit.dark.html`, `system/variables.css`, `system/theme.json`, and `logos/` synchronized. `tokens.css` activates the runtime token channel and is the only token sheet downstream artifacts load; maintainers edit `colors_and_type.css`, then regenerate both mirrors with direct declarations and byte parity.
