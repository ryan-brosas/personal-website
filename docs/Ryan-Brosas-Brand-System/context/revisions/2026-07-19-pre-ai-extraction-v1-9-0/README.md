...
Version 1.8.6 closes the Assets renderer resource boundary as well as its file path. `showcase-landing-page.html` remains the root-level Library-index mirror. `system/artifacts/landing.html` is generated from the same canonical page with its stylesheet, logo, and illustration URLs rebased two levels to the package root so the fixed renderer slot loads the complete design instead of unstyled HTML. `ryan-brosas-landing-page.html` remains the only independently edited source.
