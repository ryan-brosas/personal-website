...
> Package version: `1.9.8` — the canonical asset set now includes a registered 1080 × 1350 social campaign poster while preserving the landing page as primary  
...
The approved clean-print hero and sampled operational illustrations were also re-checked at `1254 × 1254` with ARGB transparency. The client pitch deck was re-measured at `54,138` bytes with 18 labelled slides, 95 inspectable `data-od-id` hooks, seven distinct approved illustration references, one active slide, and six written Open labels. It uses the named hero, section, campaign, and presentation roles with fixed-canvas minimums of `118px`, `72px`, `104px`, and `112px`; this prevents the host viewport from shrinking slide hierarchy inside the scaled `1920 × 1080` stage. Direct retrieval of the linked Figma node and the previously observed copywriting reference was unavailable in this pass; no external value was guessed or promoted. The re-measured local HTML, CSS, SVG, palette asset, component previews, deck, and rendered evidence remain authoritative.
...
- Evidence surfaces use the complete `--evidence-open-*`, `--evidence-loading-*`, `--evidence-empty-*`, `--evidence-error-*`, `--evidence-recovery-*`, and `--evidence-verified-*` families. An empty or failed media plate names what is missing and the next available action; a Verified plate names the inspected source and keeps the claim scoped to it.
...
The canonical social campaign proof is `building-systems-social-poster.html`, a fixed 1080 × 1350 composition for “Building systems so everything doesn’t need you.” It uses the System Conductor as narrative art, not operating evidence; keeps the full illustration inside the canvas; uses the charcoal mark on its protected signal-yellow tile; and loads root `tokens.css` once. Keep this poster indexed after the primary landing page and the client-pitch asset.
...
22. Confirm Design System Assets exposes both `showcase-landing-page.html` and `founder-dependence-client-pitch-deck.html`; the landing page remains primary, `system/artifacts/landing.html` and `system/artifacts/deck.html` exist in canonical and registered packages, and every nested renderer resource resolves after its two-level path rebase.
