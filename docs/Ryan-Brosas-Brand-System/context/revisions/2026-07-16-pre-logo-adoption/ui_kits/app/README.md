...
Open `index.html` directly for review. To reuse the kit in another project, copy `app.css`, the needed files from `components/`, the chosen production SVGs from `../../assets/`, and the canonical `colors_and_type.css`. Load component scripts before `AppShell.js`, then render `window.AppShell(data, track, theme)` into an owned root element. Keep the status names and five system-map steps intact when those patterns are used.
