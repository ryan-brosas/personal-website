...
> Package version: `1.6.0` — measured source reconciliation, explicit state roles, and simultaneous light/dark Showcase proof  
...
`colors_and_type.css` is the only canonical token source. `system/variables.css` is intentionally a package entry that imports that file; do not fork a second copy of the token values. `system/theme.json` provides the machine-readable mode and role-family index.
...
Use `--field-placeholder` only for example formats. Disabled fields use `--field-disabled-bg` and remain legible.
