...
> Package version: `1.9.4` — the canonical client pitch is now explicitly indexed in Design System Assets with its registered deck sidecar, while the landing page remains the primary asset  
...
22. Confirm Design System Assets exposes both `showcase-landing-page.html` and `founder-dependence-client-pitch-deck.html`; the landing page remains primary, the renderer slot `system/artifacts/landing.html` resolves after its two-level path rebase, and the registered deck has its matching `.artifact.json` sidecar.
