...
Version 1.9.7 improves the canonical client pitch without changing the palette, content arc, fixed deck framework, or design-system id. Named hero, section, campaign, and presentation typography roles now retain fixed-canvas minimums inside the host’s scaled preview, ordinary slide titles rise to a consistent 72px floor, and supporting grids have been rebalanced to preserve the footer-safe band. The evidence taxonomy, approved imagery, landing-page primary asset, and exact seven-file runtime inventory remain unchanged.
