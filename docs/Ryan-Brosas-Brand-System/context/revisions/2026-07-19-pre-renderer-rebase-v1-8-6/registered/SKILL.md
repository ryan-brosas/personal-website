...
14. Treat `ryan-brosas-landing-page.html` as the living applied proof for the four asset layers. Improve that same file after accepted refinements, regenerate `showcase-landing-page.html` for Library indexing and `system/artifacts/landing.html` for the Assets renderer, then refresh `assets/source-previews/ryan-brosas-landing-page-applied.png`; do not create disconnected landing-page variants unless the user requests one.
