...
> Package version: `1.8.5` — the canonical landing page now feeds both the Library index and the fixed Design System Assets renderer slot, while runtime lifecycle and content-safety contracts remain synchronized  
...
`ryan-brosas-landing-page.html` is the living applied proof for the four-layer asset system. It uses the R/lightning identity, System Conductor hero, selected operational scenes, and labelled utility icons without turning narrative art into product evidence. Improve this same page continuously, regenerate `showcase-landing-page.html` for Library indexing and `system/artifacts/landing.html` for the fixed Assets renderer slot, and refresh `assets/source-previews/ryan-brosas-landing-page-applied.png` after each accepted visual revision; do not create disconnected landing-page variants unless the user explicitly asks for an alternative direction.
...
22. Confirm the Design System Assets record for `user:brand-design-system` resolves to `showcase-landing-page.html` and the renderer slot `system/artifacts/landing.html` exists; both must be generated from the canonical landing page rather than the review index.
