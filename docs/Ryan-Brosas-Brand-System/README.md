...
Version 1.9.0 re-measures the source package and tightens the runtime contract without changing the design-system id. The maintainer-facing applied kit keeps the audit-required canonical `colors_and_type.css` entry while downstream artifacts use its byte-identical `tokens.css` mirror. Segmented preferences and filters use dedicated `--control-*` roles, and both the applied kit and side-by-side mode proof visibly exercise default, read-only, disabled, invalid, validating, valid, loading, empty, error, recovery, and Verified states. Six unrelated JSX prototypes were archived outside the active seven-file component inventory. The canonical landing page and both Assets mirrors remain synchronized and independently unchanged.

Version 1.9.1 promotes the accepted typography correction into reusable system roles. Page heroes, workbench sections, campaign statements, and presentation statements now bind size, line-height, weight, tracking, and measure through dedicated `--type-*` tokens. The applied kit, typography proof, light/dark proof, and registered Showcase consume those roles instead of oversized local clamps. The user-linked copywriting reference informed only compact statement rhythm; its blocked stylesheet was not guessed or copied. Six reintroduced JSX prototypes were preserved in revision evidence and removed from both active package directories again.

Version 1.9.3 hardens the same package against text corruption discovered in its registered proof surfaces. Active canonical and registered text files now carry an explicit UTF-8 integrity contract; the renderer mirror is allowed to change asset paths only, not content bytes; and the voice proof now demonstrates the complete client-pitch sequence from operating consequence to workflow-mapping action. The measured presentation roles, dual-mode component anatomy, seven-file runtime inventory, verified palette, and named typography scale remain unchanged.

Version 1.9.4 fixes Design System Assets discovery for the canonical client pitch. The manifest now declares the 18-slide deck as a separate preview page, its deck artifact sidecar is synchronized into the registered package, and the landing page remains the primary indexed asset. The deck HTML, fixed framework, captures, palette, tokens, and runtime component inventory are unchanged.

Version 1.9.5 closes the deck renderer boundary exposed after discovery succeeded. Open Design opens deck assets through the fixed `system/artifacts/deck.html` slot, so the package now generates that mirror from the canonical client pitch and rebases its `assets/` and `logos/` references two levels to the package root. The canonical deck remains the only independently edited source.

Version 1.9.9 strengthens state and campaign ownership without changing the four pigments, design-system id, or primary landing-page asset. Loading, empty, error, and recovery evidence captions now use their own muted roles instead of inheriting the Verified family; project readiness uses the selected-control family instead of Open styling; and the canonical 1080 × 1350 social poster consumes reusable fixed campaign roles. The light/dark proof, component preview, brand-assets preview, applied-surfaces preview, machine contracts, and AI voice guidance now expose the same boundaries. The exact seven-file runtime inventory remains unchanged.

Version 1.9.10 improves the same registered campaign poster without changing its message or asset position. The System Conductor is scaled and lifted clear of a protected charcoal footer rail, export-only format metadata is replaced with durable identity language, and the mechanism note now names the complete Context → Work → Checks → Handoffs → Recovery loop. Reusable campaign roles now own art scale and footer geometry in addition to the canvas, accent field, safe area, and title. The landing page remains the primary asset and the runtime inventory remains exactly seven components.

Version 1.9.11 closes the poster renderer boundary exposed after the campaign asset was indexed. Open Design opens the poster through the fixed `system/artifacts/poster.html` slot, so the package now generates that mirror from the canonical poster and rebases its token, illustration, and logo references two levels to the package root. The canonical poster remains byte-unchanged and the package stays published under the same `user:brand-design-system` identity.

Version 1.9.12 adds `agent-systems-service-introduction-email.html` as the fourth registered applied proof. The 640px table-based email introduces Ryan’s service to overworked founders, explains the choice among an agent, script, and process change, names the complete operating loop, keeps absent workflow evidence visibly Open, and asks for one concrete reply. A synchronized `system/artifacts/email.html` mirror supports the fixed renderer while the landing page remains primary and the runtime inventory remains exactly seven components.

Version 1.9.13 adds `agent-build-breakdown-newsletter-template.html` as the fifth registered applied proof. The reusable 640px newsletter follows exploration → expectation → observed run → interesting failure → system change → evidence boundary → next question, keeps every replaceable claim visibly bracketed, and makes reply the only next action. A synchronized `system/artifacts/newsletter.html` mirror supports the fixed renderer while the landing page remains primary and the runtime inventory remains exactly seven components.

Version 1.9.14 adds `workflow-service-inquiry-form.html` as the sixth registered applied proof. The responsive intake asks where recurring work starts, where it breaks, which tools carry it, what must remain human, and what should stop returning. It validates locally, preserves entered answers through recoverable errors, avoids budget-gating and generic AI questions, and keeps the external booking destination visibly Open until an owned URL is configured. The landing page remains primary and the runtime inventory remains exactly seven components.

Version 1.9.15 closes the form renderer boundary exposed after the workflow inquiry was indexed. Open Design opens form assets through the fixed `system/artifacts/form.html` slot, so the package now generates that mirror from the canonical inquiry and rebases only its token, illustration, and logo references two levels to the package root. The canonical form remains the only independently edited source, the landing page remains primary, and the runtime inventory remains exactly seven components.

Version 1.10.0 completes the semantic boundary around feature plates, sticky navigation overlays, and disabled task actions. The applied runtime no longer reconstructs those values with local `color-mix()` or compounds disabled text with blanket opacity. Verified examples now name inspected sources, Proposed examples name tradeoffs, and Open examples name both the owned proof and the publication decision they block. The four pigments, same `user:brand-design-system` id, six applied artifacts, primary landing page, and exact seven-file runtime inventory remain unchanged.

Version 1.10.1 completes feedback-message hierarchy with dedicated neutral, error, and notice muted-detail roles. The workbench, component preview, light/dark proof, and Showcase no longer use blanket opacity for feedback body copy; Verified panels consume their registered border and Proposed panels consume their registered muted-copy role. Six reintroduced JSX prototypes were archived from the source component directory, restoring exact parity with the seven-file distribution. The same `user:brand-design-system` id and primary landing-page asset remain unchanged.

Version 1.10.2 closes the readiness gaps found beyond the baseline package audit. Twenty-four final dark-scope declarations rebind mode-dependent semantic, compatibility, rule, shadow, and elevation aliases for nested kits; `system/theme.json` now inventories all 309 unique token names; and `system/kit.dark.html` opens the relative dark entry instead of recursively targeting the Showcase API. Typography, spacing, and radius specimens now demonstrate the same named roles and component families downstream projects are expected to use. The registered id, primary landing page, six applied artifacts, and exact seven-component runtime remain unchanged.

Version 1.10.3 gives the five-part System Map a complete ten-role component family instead of rebuilding it from generic panels and surfaces. The light/dark proof now renders identical map anatomy in both modes, machine contracts include reusable operating-copy grammar, and the voice proof replaces an unconfigured booking action with a real reply action. Six reintroduced JSX chat prototypes were archived again, restoring the exact seven-file runtime without changing `user:brand-design-system`, the primary landing page, or the six registered applied artifacts.
